...
def urgencyIndexer(obj):
    ...
    if due_date != None:
        ...
        if workdays <=   red:
            return "1 Urgent < {:02d} workdays".format(red)

        if workdays <=   yellow:
            return "2 Soon < {:02d} workdays".format(yellow) 

        if workdays <=   green:
            return "3 Future < {days} workdays".format(days = green) 

        return '4 More than ' + green
