...
class ViewsIntegrationTest(unittest.TestCase):
    ...
    def test_action_items_view_is_registered(self):
        view = getMultiAdapter(
            (self.portal['other-folder'], self.portal.REQUEST),
            name='action-items-view'
        )
        self.assertTrue(view.__name__ == 'action-items-view')
        # self.assertTrue(
        #     'Sample View' in view(),
        #     'Sample View is not found in action-items-view'
        # )

    def test_action_items_view_not_matching_interface(self):
